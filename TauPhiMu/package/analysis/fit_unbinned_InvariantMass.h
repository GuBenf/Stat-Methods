...
Double_t pdf_proj_iv(Double_t *x, Double_t *par, Int_t max, Double_t bin_width)
{
...
void fit_unbinned_InvariantMass(std::vector<double> input_xvar, std::vector<double> vstart, std::vector<double> step, std::vector<double> low, std::vector<double> high,  std::vector<std::string> par_names, TH1D *h_time, TFile *file, std::string name, double xmin, double xmax)
{
     x_var = input_xvar;
     ...
     const int n_fit = x_var.size();
      ...
      { 
            ...
            y_points.push_back(pdf_proj_iv(&x,pars,n_fit,h_time->GetBinWidth(1)));
      }
      ...
      for (int i = 0; i < n; i++)
      {
          ...
          double y_fit = pdf_proj_iv(&x, pars, n_fit, h_time->GetBinWidth(1));
...
std::tuple <Double_t, Double_t> fit_unbinned_InvariantMass_results(std::vector<double> input_xvar, std::vector<double> vstart, std::vector<double> step, std::vector<double> low, std::vector<double> high,  std::vector<std::string> par_names, TH1D *h_time, TFile *file, std::string name, double xmin, double xmax)
{
     x_var = input_xvar;
