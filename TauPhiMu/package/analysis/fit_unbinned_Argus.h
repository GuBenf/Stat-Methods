double pdf_Argus ( double *x , double * par )
{
  ...
  double L_taumass_range = 1.6;
  double H_taumass_range = 2.;
...
{
  return (max*bin_width)*pdf_Argus(&x[0],par);
}
...
void fcn(Int_t &, Double_t *, Double_t &f, Double_t *par, Int_t )
{
  ...
  for(size_t i=0; i<x_var.size(); ++i)
    {
      Double_t p = pdf_Argus(&x_var[i],par);
      Like += TMath::Log(p);
  };
   f= - 2. * Like;
}

void fit_unbinned_Argus(std::vector<double> input_xvar, std::vector<double> vstart, std::vector<double> step, std::vector<double> low, std::vector<double> high, std::vector<std::string> par_names, TH1D *h_time, TFile *file, std::string name, double xmin, double xmax)
{
     x_var = input_xvar;
