...
Double_t pdf_exp_x_gauss(Double_t x, Double_t *par)
{
   ...
   //muB = MU_B_FIXED;
   sigmaB = S_B_FIXED;
   signal_fraction = par[1];
   tau_B = TAU_BACKGROUND;

   Double_t signalA = fA*( 1./(2*tau)*TMath::Exp((sigmaA*sigmaA - 2*tau*x)/(2*tau*tau))*( TMath::Erf((tau*x - sigmaA*sigmaA)/(TMath::Sqrt(2)*tau*sigmaA)) +1 ) );
   
   Double_t signal_B = fB*( 1./(2*tau)*TMath::Exp((sigmaB*sigmaB - 2*tau*x)/(2*tau*tau))*( TMath::Erf((tau*x - sigmaB*sigmaB)/(TMath::Sqrt(2)*tau*sigmaB)) +1 ) );

   Double_t bkg_A = fA*( 1./(2*tau_B)*TMath::Exp((sigmaA*sigmaA - 2*tau_B*x)/(2*tau_B*tau_B))*( TMath::Erf((tau_B*x - sigmaA*sigmaA)/(TMath::Sqrt(2)*tau_B*sigmaA)) +1 ) );
   
   Double_t bkg_B = fB*( 1./(2*tau_B)*TMath::Exp((sigmaB*sigmaB - 2*tau_B*x)/(2*tau_B*tau_B))*( TMath::Erf((tau_B*x - sigmaB*sigmaB)/(TMath::Sqrt(2)*tau_B*sigmaB)) +1 ) );

   Double_t value = signal_fraction * (signalA + signal_B) + (1 - signal_fraction)* (bkg_A + bkg_B);
...
{  
      ...
      { 
          ...
          */
          if(M0_MKpi > 1.86482 - 3 * 0.00521984 && M0_MKpi < 1.86482 + 3 * 0.00521984)
          {
      ...
      Double_t vstart[nparam] = {1., 0.5};
      Double_t step[nparam]   = {0.01, 0.01};   //step 0 li rende costanti
     
      my_gMinuit->mnparm(0, "tau", vstart[0], step[0], 0.1, 10., ierflg);
      my_gMinuit->mnparm(1, "signal_fraction", vstart[1], step[1], 0., 1., ierflg);
      ...
      Double_t tau_fit, etau_fit, signal_fraction_fit, e_signal_fraction_fit, bnd1, bnd2;
      Int_t ivar=0;
      TString chnam;
